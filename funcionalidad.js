...
window.toggleHamburgerDropdown = function(event) {
...
window.closeHamburgerDropdown = function() {
    const dropdown = document.getElementById('hamburgerDropdownMenu');
    if (dropdown) {
...
function setupDelegatedDragEvents() {
    ...
    tbody.addEventListener('touchmove', (e) => {
        if (!isDragging) { const currentY = e.touches[0].clientY; if (Math.abs(currentY - startY) > 10) clearTimeout(pressTimer); } 
    ...
    tbody.addEventListener('touchcancel', () => { clearTimeout(pressTimer); if (draggingRow) draggingRow.classList.remove('dragging'); isDragging = false; draggingRow = null; });
}

function saveNewOrder() {
    const rows = document.querySelectorAll('#tableBody tr');
    const newPlaylist = [];
    rows.forEach(row => { const id = row.getAttribute('data-index'); if (id !== null) newPlaylist.push(parseInt(id)); });
...
window.connectToSession = function() {
    ...
    currentUserKey = codeInput; sessionStorage.setItem('acordify_user_key', currentUserKey); isConnected = true;
    const roomRef = ref(db, 'sessions/' + FIXED_ROOM_ID); const chatRef = ref(db, 'chats/' + FIXED_ROOM_ID); const connectionsRef = ref(db, 'connections/' + FIXED_ROOM_ID);
    myConnectionRef = push(connectionsRef); set(myConnectionRef, currentUserKey); onDisconnect(myConnectionRef).remove();
...
function reconnectSession() {
    ...
    const roomRef = ref(db, 'sessions/' + FIXED_ROOM_ID); const connectionsRef = ref(db, 'connections/' + FIXED_ROOM_ID);
    myConnectionRef = push(connectionsRef); set(myConnectionRef, currentUserKey); onDisconnect(myConnectionRef).remove();
    startListening(roomRef); startChatListener(); updateUIConnected(); broadcastChange();
}

function startListening(roomRef) {
    onValue(roomRef, (snapshot) => {
        const cloudPlaylist = snapshot.val() || []; const cloudString = JSON.stringify(cloudPlaylist);
        if (cloudString !== JSON.stringify(myPlaylist) && cloudString !== lastCloudPlaylistString) {
            myPlaylist = cloudPlaylist; localStorage.setItem('myPlaylist', JSON.stringify(myPlaylist)); lastCloudPlaylistString = cloudString;
            if (window.location.pathname.includes('lista.html')) { window.loadPlaylistMode(); } else if (!window.location.hash) { document.querySelectorAll('.btn-add').forEach(btn => { const rowId = parseInt(btn.id.split('-')[1]); if (myPlaylist.includes(rowId)) { btn.innerText = "✓"; btn.classList.add('added'); btn.onclick = null; } else { btn.innerText = "+"; btn.classList.remove('added'); btn.setAttribute('onclick', `addToPlaylist(${rowId}, this)`); } }); }
        }
    });
}
...
function startChatListener() {
    const chatRef = ref(db, 'chats/' + FIXED_ROOM_ID); const chatBox = document.getElementById('chatMessages'); const badge = document.getElementById('chatBadge'); if (!chatBox) return; chatBox.innerHTML = '';
    onChildAdded(chatRef, (snapshot) => { const msg = snapshot.val(); renderMessage(msg); if(!isChatOpen) { unreadMessages++; if(badge) { badge.innerText = unreadMessages; badge.style.display = 'flex'; } } else { if(chatBox) chatBox.scrollTop = chatBox.scrollHeight; } });
}

function renderMessage(msg) {
    const chatBox = document.getElementById('chatMessages'); if(!chatBox) return;
    const isMe = msg.user === currentUserKey; const div = document.createElement('div');
    let bubbleClass = 'message-bubble'; if (msg.type === 'STICKER') bubbleClass += ' msg-sticker';
    if (isMe) { bubbleClass += ' msg-me'; } else { bubbleClass += ' msg-other'; if (msg.user === 'SOL') bubbleClass += ' user-SOL'; else if (msg.user === 'SAM') bubbleClass += ' user-SAM'; else if (msg.user === 'PASTOR') bubbleClass += ' user-PASTOR'; else if (msg.user === 'SAMU') bubbleClass += ' user-SAMU'; else if (msg.user === 'ANGEL') bubbleClass += ' user-ANGEL'; else if (msg.user === 'CRIS') bubbleClass += ' user-CRIS'; else bubbleClass += ' user-UNKNOWN'; }
    div.className = bubbleClass;
    let contentHtml = ''; if (msg.type === 'STICKER') contentHtml = `<img src="${msg.text}" alt="sticker" loading="lazy">`; else contentHtml = msg.text;
    div.innerHTML = `<span class="msg-sender">${isMe ? 'Tú' : msg.user}</span>${contentHtml}<span class="msg-time">${msg.time}</span>`; chatBox.appendChild(div);
}

/* --- UTILIDADES MODALES --- */
const filterKeys = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
window.openKeyModal = function() { const modal = document.getElementById('keyModal'); if(modal) modal.style.display = 'flex'; }
window.closeKeyModal = function() { const modal = document.getElementById('keyModal'); if(modal) modal.style.display = 'none'; }
window.openSubKeyModal = function(baseKey) {
    window.closeKeyModal(); let subModal = document.getElementById('subKeyModal');
    if (!subModal) {
        subModal = document.createElement('div'); subModal.id = 'subKeyModal'; subModal.className = 'modal';
    ...
    const btnMajor = document.getElementById('btnMajorKey'); btnMajor.innerText = baseKey; btnMajor.onclick = function() { window.filterByKey(baseKey); window.closeSubKeyModal(); };
    const btnMinor = document.getElementById('btnMinorKey'); btnMinor.innerText = baseKey + 'm'; btnMinor.onclick = function() { window.filterByKey(baseKey + 'm'); window.closeSubKeyModal(); };
    subModal.style.display = 'flex';
}
window.closeSubKeyModal = function() { const subModal = document.getElementById('subKeyModal'); if (subModal) subModal.style.display = 'none'; }
window.generateKeyButtons = function() { const grid = document.getElementById('keyGrid'); if (!grid) return; grid.innerHTML = ''; filterKeys.forEach(key => { let btn = document.createElement('button'); btn.className = 'key-btn'; btn.innerText = key; btn.onclick = function() { window.openSubKeyModal(key); }; grid.appendChild(btn); }); }
...
function handleSwipeGesture(startX, endX, startY, endY) {
    const diffX = startX - endX; const diffY = startY - endY; const absDiffX = Math.abs(diffX); const absDiffY = Math.abs(diffY); const minSwipeDistance = 60; 
    if (absDiffX > absDiffY && absDiffX > minSwipeDistance) { if (diffX > 0) { changeSongInPlaylist(1); } else { changeSongInPlaylist(-1); } }
}

function changeSongInPlaylist(direction) {
    if (!currentContextList || currentContextList.length === 0) return; let currentPos = currentContextList.indexOf(currentSongIndex); if (currentPos === -1) return; 
    let newPos = currentPos + direction; if (newPos >= currentContextList.length) newPos = 0; else if (newPos < 0) newPos = currentContextList.length - 1; 
    let nextSongID = currentContextList[newPos]; window.openSong(nextSongID);
}